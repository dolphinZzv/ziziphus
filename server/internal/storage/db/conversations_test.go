...
func TestConvRepo_Get(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT conv_id, type, name, owner_id, avatar, cover, notice, max_members, last_msg_id, last_msg_at, created_at FROM conversations WHERE conv_id = $1`)).
		WithArgs("conv1").
		WillReturnRows(pgxmock.NewRows([]string{"conv_id", "type", "name", "owner_id", "avatar", "cover", "notice", "max_members", "last_msg_id", "last_msg_at", "created_at"}).
			AddRow("conv1", model.ConvP2P, "test", "u1", "av.jpg", "cv.jpg", "notice", 2, int64(100), &now, now))
...
func TestConvRepo_Get_NoLastMsg(t *testing.T) {
	...
	mock.ExpectQuery(`FROM conversations WHERE conv_id`).
		WithArgs("conv_new").
		WillReturnRows(pgxmock.NewRows([]string{"conv_id", "type", "name", "owner_id", "avatar", "cover", "notice", "max_members", "last_msg_id", "last_msg_at", "created_at"}).
			AddRow("conv_new", model.ConvP2P, "new", "u1", "", "", "", 2, nil, nil, now))
