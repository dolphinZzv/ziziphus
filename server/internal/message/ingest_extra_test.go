...
func newIngestFixtureWithContactReq(ratePerSec, burst, maxBody int, defaultID int64, contactReqDB contactRequestDB) *Ingest {
	...
	return NewIngest(store, router, pusher, rateLmt, idGen, seqCache, convMgr, contactReqDB, &mockContactCreator{})
}
...
func TestHandleFormResponse_ApproveSuccess(t *testing.T) {
	...
	ing := NewIngest(store, router, pusher, rateLmt, idGen, seqCache, convMgr, crdb, &mockContactCreator{})
