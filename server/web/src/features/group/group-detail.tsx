...
import { X, Crown, Shield, Trash2, Check, X as XIcon, Camera, Search, Cpu, UserPlus, Bell, Edit2, EyeOff, FileUp, Globe, Plus, Trash2 as TrashIcon, Key, Shield, Activity, RefreshCw, CheckCircle, XCircle, Clipboard } from 'lucide-react'
...
export default function GroupDetail({ convId, onClose }: Props) {
  ...
  const [editingWebhook, setEditingWebhook] = useState<ConvWebhook | null>(null)
  const [whForm, setWhForm] = useState({ name: '', callback_url: '', headers: [] as { key: string; value: string }[], cidr_whitelist: '', require_audit: false })
  const [whCreated, setWhCreated] = useState<{ token: string; api_key: string } | null>(null)
  ...
  const openCreateForm = () => {
    ...
    setWhForm({ name: '', callback_url: '', headers: [], cidr_whitelist: '', require_audit: false })
    setShowWebhookForm(true)
  }

  const openEditForm = (wh: ConvWebhook) => {
    setEditingWebhook(wh)
    setWhForm({
      name: wh.name,
      callback_url: wh.callback_url || '',
      headers: wh.headers || [],
      cidr_whitelist: (wh.cidr_whitelist || []).join(', '),
      require_audit: wh.require_audit,
    })
    setShowWebhookForm(true)
  }
  ...
  const handleWebhookSave = async () => {
    ...
    try {
      const data = {
        name: whForm.name,
        callback_url: whForm.callback_url,
        headers: whForm.headers,
        cidr_whitelist: whForm.cidr_whitelist ? whForm.cidr_whitelist.split(',').map(s => s.trim()).filter(Boolean) : [],
        require_audit: whForm.require_audit,
      }
      if (editingWebhook) {
        await webhookService.update(convId, editingWebhook.id, data)
        ...
        setWhCreated({ token: result.token, api_key: result.api_key })
      }
  ...
  const handleWebhookDelete = async (id: number) => {
    ...
    try {
      await webhookService.delete(convId, id)
      setWebhooks(prev => prev.filter(w => w.id !== id))
    } catch {}
  }

  const handleRegenerateKey = async (id: number) => {
    if (!confirm(t('conversation.webhookRegenerateConfirm'))) return
    try {
      const result = await webhookService.regenerateKey(convId, id)
      setWhCreated({ token: '', api_key: result.api_key })
    } catch {}
  }
          ...
          <div className="border-t border-[var(--color-hairline)] pt-3 mt-3">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-[var(--color-muted)]">{t('conversation.webhookTitle')}</span>
              </div>
              <button onClick={openCreateForm}
                className="p-1 rounded-xl hover:bg-[var(--color-hairline)] text-[var(--color-muted)] hover:text-[var(--color-accent)] transition-colors">
                <Plus size={14} />
              </button>
            </div>
            ...
            {/* Webhook list */}
            {webhooks.length === 0 && (
              <div className="text-[10px] text-[var(--color-muted-soft)] text-center py-2">{t('conversation.webhookNoData')}</div>
            )}
            {webhooks.map(wh => (
              <div key={wh.id} className="flex items-start gap-2 py-2 border-b border-[var(--color-hairline-soft)] last:border-0">
                <div className="flex-1 min-w-0">
                  ...
                  {wh.callback_url && (
                    <div className="text-[10px] text-[var(--color-muted)] truncate">{wh.callback_url}</div>
                  )}
                  <div className="flex items-center gap-2 mt-1 text-[9px] text-[var(--color-muted-soft)]">
                    <span className="flex items-center gap-0.5"><Key size={9} /> {t('conversation.webhookKey')}</span>
                    {wh.cidr_whitelist && wh.cidr_whitelist.length > 0 && (
                      <span className="flex items-center gap-0.5"><Shield size={9} /> {t('conversation.webhookCIDRLabel')}</span>
                    )}
                    {wh.require_audit && (
                      <span className="flex items-center gap-0.5"><Activity size={9} /> {t('conversation.webhookAuditLabel')}</span>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button onClick={() => openEditForm(wh)}
                    className="p-1 rounded hover:bg-[var(--color-hairline)] text-[var(--color-muted)] hover:text-[var(--color-accent)]">
                  ...
                  <button onClick={() => handleWebhookDelete(wh.id)}
                    className="p-1 rounded hover:bg-[var(--color-hairline)] text-[var(--color-muted)] hover:text-[var(--destructive)]">
                    <TrashIcon size={12} />
                  </button>
                </div>
              </div>
            ...
            {/* Add/Edit form modal */}
            {showWebhookForm && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => setShowWebhookForm(false)}>
                <div className="w-[380px] bg-[var(--color-surface-card)] rounded-xl p-4" style={{ boxShadow: 'var(--shadow-lg)' }} onClick={e => e.stopPropagation()}>
                  <div className="text-sm font-medium text-[var(--color-ink)] mb-3">
                    {editingWebhook ? t('conversation.webhookEdit') : t('conversation.webhookAdd')}
                  </div>
                  <div className="space-y-2.5">
                    <div>
                      ...
                      <input value={whForm.name} onChange={e => setWhForm({ ...whForm, name: e.target.value })}
                        className="w-full h-8 px-2 rounded-lg text-xs bg-[var(--color-surface-soft)] border border-[var(--color-hairline)] focus:outline-none focus:border-[var(--color-primary)]" />
                    ...
                    <div>
                      ...
                      <input value={whForm.callback_url} onChange={e => setWhForm({ ...whForm, callback_url: e.target.value })}
                        className="w-full h-8 px-2 rounded-lg text-xs bg-[var(--color-surface-soft)] border border-[var(--color-hairline)] focus:outline-none focus:border-[var(--color-primary)]" />
                    ...
                    <div>
                      ...
                      <input value={whForm.cidr_whitelist} onChange={e => setWhForm({ ...whForm, cidr_whitelist: e.target.value })}
                        className="w-full h-8 px-2 rounded-lg text-xs bg-[var(--color-surface-soft)] border border-[var(--color-hairline)] focus:outline-none focus:border-[var(--color-primary)]" />
                      <div className="text-[9px] text-[var(--color-muted-soft)] mt-0.5">{t('conversation.webhookCIDRHint')}</div>
                    </div>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" checked={whForm.require_audit}
                        onChange={e => setWhForm({ ...whForm, require_audit: e.target.checked })}
                    ...
                    {!editingWebhook && (
                      <div className="text-[10px] text-[var(--color-muted-soft)] bg-[var(--color-surface-soft)] rounded-lg p-2">
                        {t('conversation.webhookCreatedOnce')}
                      </div>
                    )}
                  </div>
                  <div className="flex items-center justify-end gap-2 mt-4">
                    <button onClick={() => setShowWebhookForm(false)}
                      className="px-3 py-1.5 rounded-lg text-xs text-[var(--color-muted)] hover:bg-[var(--color-hairline)]">{t('common.cancel')}</button>
                    <button onClick={handleWebhookSave}
                      className="px-3 py-1.5 rounded-lg text-xs bg-[var(--color-primary)] text-white hover:bg-[var(--color-primary-hover)]">{t('common.save')}</button>
                  </div>
                </div>
              </div>
            )}

            {/* Secret display modal (create/regenerate) */}
            {whCreated && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => { setWhCreated(null) }}>
                <div className="w-[380px] bg-[var(--color-surface-card)] rounded-xl p-4" style={{ boxShadow: 'var(--shadow-lg)' }} onClick={e => e.stopPropagation()}>
                  ...
                  {whCreated.token && (
                    <div className="flex items-center justify-between mb-2">
                      <div className="text-xs font-mono text-[var(--color-ink)] select-all flex-1 truncate mr-2">{whCreated.token}</div>
                      <button onClick={() => navigator.clipboard.writeText(whCreated.token)}
                        className="p-1 rounded hover:bg-[var(--color-hairline)] text-[var(--color-muted)]"><Clipboard size={14} /></button>
                    </div>
                  )}
                  <div className="flex items-center justify-between mb-3">
                    <div className="text-xs font-mono text-[var(--color-ink)] select-all flex-1 truncate mr-2">{whCreated.api_key}</div>
                    <button onClick={() => navigator.clipboard.writeText(whCreated.api_key)}
                      className="p-1 rounded hover:bg-[var(--color-hairline)] text-[var(--color-muted)]"><Clipboard size={14} /></button>
                  </div>
                  <button onClick={() => setWhCreated(null)}
                    className="w-full h-8 rounded-lg text-xs bg-[var(--color-primary)] text-white hover:bg-[var(--color-primary-hover)]">{t('common.close')}</button>
                </div>
              </div>
            )}
          </div>
          ...
          {pendingMsgs.length > 0 && (
            <div className="border-t border-[var(--color-hairline)] pt-3 mt-3">
              <button onClick={() => setShowPending(!showPending)}
                className="flex items-center gap-2 text-xs font-medium text-[var(--color-muted)] mb-1">
              ...
              {showPending && pendingMsgs.map(pm => (
                <div key={pm.msg_id} className="flex items-center gap-2 py-1.5 border-b border-[var(--color-hairline-soft)] last:border-0">
                  ...
                  <button onClick={() => handleApprove(pm.msg_id)}
                    className="p-1 rounded hover:bg-[var(--color-hairline)] text-[var(--color-accent)]"><CheckCircle size={14} /></button>
                  <button onClick={() => handleReject(pm.msg_id)}
                    className="p-1 rounded hover:bg-[var(--color-hairline)] text-[var(--destructive)]"><XCircle size={14} /></button>
                </div>
              ))}
            </div>
          )}
