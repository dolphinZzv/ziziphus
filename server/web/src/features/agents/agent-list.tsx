...
import { X, Plus, Edit, Trash2, Key, Copy, Check, Camera, Cpu } from 'lucide-react'
import { cn } from '@/lib/cn'

interface Props { onClose: () => void }

export default function AgentList({ onClose }: Props) {
      ...
      <div className="w-[420px] max-h-[520px] bg-[var(--color-surface-card)] border border-[var(--color-hairline)] rounded-lg p-6 flex flex-col"
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-1">
            {agents.length < 10 && <button onClick={() => setShowCreate(true)} className="p-1.5 rounded-lg hover:bg-[var(--color-surface-soft)] text-[var(--color-muted)]"><Plus size={16} /></button>}
        ...
        <div className="flex-1 overflow-y-auto space-y-0.5">
          {agents.map(agent => (
            <div key={agent.user_id} className="flex items-center gap-3 px-3 h-12 rounded-lg hover:bg-[var(--color-surface-soft)] group">
              ...
              <button onClick={() => setEditing(agent)} className="p-1.5 rounded-lg hover:bg-[var(--color-surface-soft)] opacity-0 group-hover:opacity-100 text-[var(--color-muted)] transition-all"><Edit size={14} /></button>
              <button onClick={() => handleDelete(agent.user_id)} className="p-1.5 rounded-lg hover:bg-[var(--destructive)]/10 opacity-0 group-hover:opacity-100 text-[var(--destructive)] transition-all"><Trash2 size={14} /></button>
            </div>
          ))}
...
function AgentEditDialog({ agent, onClose, onSaved }: { agent: User | null; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/30 rounded-lg" onClick={onClose}>
      <div className="w-[360px] bg-[var(--color-surface-card)] border border-[var(--color-hairline)] rounded-lg p-6"
        style={{ boxShadow: 'var(--shadow-lg)' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Agent 名称" className={inputClass} />
          </div>
          <div>
          ...
          {agent && apiKey && (
            <div>
              ...
              <div className="flex gap-1">
                <input type={showKey ? 'text' : 'password'} value={apiKey} readOnly className={cn(inputClass, 'font-mono text-xs')} />
                <button onClick={() => setShowKey(!showKey)} className="px-2 text-xs text-[var(--color-muted)]">{showKey ? '隐藏' : '显示'}</button>
                <button onClick={copyKey} className="p-2 rounded-lg hover:bg-[var(--color-surface-soft)]">{copied ? <Check size={14} className="text-[var(--success)]" /> : <Copy size={14} className="text-[var(--color-muted)]" />}</button>
              </div>
              <button onClick={handleRegenerateKey} className="text-[11px] text-[var(--destructive)] mt-1 hover:underline">重新生成</button>
            </div>
          )}
          <div className="flex gap-2 pt-2">
            <button onClick={onClose} className="flex-1 h-[42px] rounded-lg border border-[var(--color-hairline)] text-sm text-[var(--color-body)] hover:bg-[var(--color-surface-soft)] transition-colors">取消</button>
            <button onClick={handleSave} disabled={saving} className="flex-1 h-[42px] rounded-lg bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] text-white text-sm font-medium transition-colors disabled:opacity-40">{saving ? '保存中...' : '保存'}</button>
          </div>
        </div>
      </div>
    </div>
