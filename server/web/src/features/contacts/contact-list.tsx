...
import { X, UserPlus, MessageCircle, Trash2, Search, Check, ArrowRight } from 'lucide-react'
      ...
      <div className="w-[380px] max-h-[500px] bg-[var(--color-surface-card)] border border-[var(--color-hairline)] rounded-lg p-6 flex flex-col"
        ...
        {showSearch && (
          <>
            ...
            {successMsg && (
              <div className="mb-3 flex items-center justify-between px-3 py-2 rounded-lg bg-green-500/10 text-xs text-green-600">
                <span className="flex items-center gap-1"><Check size={12} /> {successMsg}</span>
                <button onClick={() => { navigate(`/chat/sys:${me?.user_id}`); onClose() }}
        ...
        <div className="flex-1 overflow-y-auto space-y-0.5">
          {contacts.filter(contact => {
            ...
            return (
              <div key={contact.user_id} className="flex items-center gap-3 px-3 h-12 rounded-lg hover:bg-[var(--color-surface-soft)] group">
                <div className="relative">
