    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={onClose}>
      <div className="w-[400px] max-h-[480px] bg-[var(--color-surface-card)] border border-[var(--color-hairline)] rounded-lg p-6 flex flex-col"
        ...
        <div className="flex-1 overflow-y-auto space-y-0.5">
          {results.map(group => (
            <div key={group.conv_id} className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-[var(--color-surface-soft)]">
              <div className="w-9 h-9 rounded-lg flex items-center justify-center text-white text-sm font-semibold flex-shrink-0"
              ...
              <button onClick={() => handleJoin(group.conv_id)}
                className="px-3 py-1.5 rounded-lg border border-[var(--color-primary)] text-[var(--color-primary)] text-xs font-medium hover:bg-[var(--color-primary)]/5 transition-colors">
                加入
              </button>
            </div>
          ))}
