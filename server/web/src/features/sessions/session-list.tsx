...
export default function SessionList({ onClose, inline }: Props) {
  ...
  const currentSessionId = useSyncExternalStore(authStore.subscribe, () => authStore.state.sessionId)
  ...
  const inner = (
    <div className={`${inline ? 'h-full' : 'w-full sm:w-[400px] h-full sm:h-auto max-h-[100dvh] sm:max-h-[calc(100vh-80px)]'} bg-[var(--color-surface-card)] border border-[var(--color-hairline)] rounded-none sm:rounded-xl p-6 flex flex-col overflow-hidden`}
